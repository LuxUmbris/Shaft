...
This document specifies the syntax of the current [LLVM bootstrap compiler](github.com/LuxUmbris/Shaft/tree/bootstrap).
...
`:` is used by `foreach` and enum underlying-type syntax. `...` is an inclusive integer range in `foreach`, for example `foreach (i64 i : 1...6) { ... }` visits `1` through `6` in ascending order.
...
`using` is private to its source module and lexical scope. `using global` exports an alias to later compiler source modules. The standard prelude uses `using global` for its public output convenience macros: `print!`, both `println!` arities, and formatted `printf!`; they are available from application source. Its other internal macros remain private. Local declarations shadow global ones. An invocation of an unavailable `name!(...)` is a lexer error, preventing accidental fallback to an ordinary expression.
